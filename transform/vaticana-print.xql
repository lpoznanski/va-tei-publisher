...
module namespace model="http://www.tei-c.org/pm/models/vaticana/print";
    ...
    $input !         (
            ...
            return
                    ...
                    case element(persName) return
                        html:inline($config, ., ("tei-persName", "annotation", "annotation-role", "authority", css:map-rend-to-class(.)), .)
                    case element(settlement) return
                        printcss:alternate($config, ., ("tei-settlement", css:map-rend-to-class(.)), ., ., .)
                    case element(affiliation) return
                        html:link($config, ., ("tei-affiliation", css:map-rend-to-class(.)), ., concat('https://va.wiki.kul.pl/wiki/Item:', substring-after(@ref, '-')), (), map {})
                    case element(measure) return
                        printcss:alternate($config, ., ("tei-measure", css:map-rend-to-class(.)), ., ., concat('Jednostka: ', @unit, ', Ilość: ', @quantity, ', Szczegóły: ', @commodity))
                    case element(orgName) return
                        printcss:alternate($config, ., ("tei-orgName", css:map-rend-to-class(.)), ., ., .)
                    ...
                    case element(section) return
                        if (@type='text') then
                            html:inline($config, ., ("tei-section1", css:map-rend-to-class(.)), .)
                        else
                            if (@type='formularz') then
                                html:inline($config, ., ("tei-section2", css:map-rend-to-class(.)), .)
                            else
                                $config?apply($config, ./node())
                    case element(education) return
                        printcss:alternate($config, ., ("tei-education", css:map-rend-to-class(.)), ., ., @subtype)
