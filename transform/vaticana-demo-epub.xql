...
module namespace model="http://www.tei-c.org/pm/models/vaticana-demo/epub";
            ...
            return
                    ...
                    case element(teiHeader) return
                        if ($parameters?header='short') then
                            ...
                            (
                                epub:block($config, ., ("tei-teiHeader4", css:map-rend-to-class(.)), //publicationStmt/p),
                    ...
                    case element(body) return
                        (
                            epub:block($config, ., ("tei-body1", css:map-rend-to-class(.)), root(.)//fileDesc/titleStmt/substring-before(title, '-')),
                            ...
                            epub:block($config, ., ("tei-body3", css:map-rend-to-class(.)), root(.)//profileDesc/abstract),
                    ...
                    case element(persName) return
                        epub:alternate($config, ., ("tei-persName", css:map-rend-to-class(.)), ., ., .)
                    case element(affiliation) return
                        epub:alternate($config, ., ("tei-affiliation", css:map-rend-to-class(.)), ., ., .)
                    case element(placeName) return
                        if (parent::p) then
                            epub:alternate($config, ., ("tei-placeName", css:map-rend-to-class(.)), ., ., .)
                        else
                            $config?apply($config, ./node())
