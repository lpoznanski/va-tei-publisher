    ...
    $input !         (
            ...
            return
                    ...
                    case element(persName) return
                        html:inline($config, ., ("tei-persName", "annotation", "annotation-role", "authority", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
                    case element(settlement) return
                        html:alternate($config, ., ("tei-settlement", css:map-rend-to-class(.)), ., ., ., map {})                        => model:map($node, $trackIds)
                    case element(affiliation) return
                        html:link($config, ., ("tei-affiliation", css:map-rend-to-class(.)), ., concat('https://va.wiki.kul.pl/wiki/Item:', substring-after(@ref, '-')), (), map {})                        => model:map($node, $trackIds)
                    case element(measure) return
                        html:alternate($config, ., ("tei-measure", css:map-rend-to-class(.)), ., ., concat('Jednostka: ', @unit, ', Ilość: ', @quantity, ', Szczegóły: ', @commodity), map {})                        => model:map($node, $trackIds)
                    case element(orgName) return
                        html:alternate($config, ., ("tei-orgName", css:map-rend-to-class(.)), ., ., ., map {})                        => model:map($node, $trackIds)
                    ...
                    case element(section) return
                        if (@type='text') then
                            html:inline($config, ., ("tei-section1", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                        else
                            if (@type='formularz') then
                                html:inline($config, ., ("tei-section2", css:map-rend-to-class(.)), .)                                => model:map($node, $trackIds)
                            else
                                $config?apply($config, ./node())
                    case element(education) return
                        html:alternate($config, ., ("tei-education", css:map-rend-to-class(.)), ., ., @subtype, map {})                        => model:map($node, $trackIds)
