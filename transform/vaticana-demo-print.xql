...
module namespace model="http://www.tei-c.org/pm/models/vaticana-demo/print";
            ...
            return
                    ...
                    case element(teiHeader) return
                        if ($parameters?header='short') then
                            ...
                            (
                                html:block($config, ., ("tei-teiHeader4", css:map-rend-to-class(.)), //publicationStmt/p),
                                html:listItem($config, ., ("tei-teiHeader5", css:map-rend-to-class(.)), //sourceDesc, 'Źródła:'),
                                html:listItem($config, ., ("tei-teiHeader6", css:map-rend-to-class(.)), //date, 'Data wydania:'),
                                html:listItem($config, ., ("tei-teiHeader7", css:map-rend-to-class(.)), //placeName, 'Miejsce wydania:')
                            )
                    ...
                    case element(p) return
                        if (parent::abstract) then
                            ...
                            if (ancestor::note) then
                                html:inline($config, ., ("tei-p2", css:map-rend-to-class(.)), .)
                            else
                    ...
                    case element(body) return
                        (
                            html:block($config, ., ("tei-body1", css:map-rend-to-class(.)), root(.)//fileDesc/titleStmt/substring-before(title, '-')),
                            html:link($config, ., ("tei-body2", css:map-rend-to-class(.)), root(.)//fileDesc/titleStmt/substring-after(title, '- '), concat('https://va.wiki.kul.pl/wiki/Item:', root(.)//fileDesc/titleStmt/substring-after(title, '- ')), (), map {}),
                            html:block($config, ., ("tei-body3", css:map-rend-to-class(.)), root(.)//profileDesc/abstract),
                            html:section($config, ., ("tei-body4", css:map-rend-to-class(.)), .)
                        )
                    ...
                    case element(persName) return
                        printcss:alternate($config, ., ("tei-persName", css:map-rend-to-class(.)), ., ., .)
                    case element(affiliation) return
                        printcss:alternate($config, ., ("tei-affiliation", css:map-rend-to-class(.)), ., ., .)
                    case element(placeName) return
                        if (parent::p) then
                            printcss:alternate($config, ., ("tei-placeName", css:map-rend-to-class(.)), ., ., .)
                        else
                            $config?apply($config, ./node())
