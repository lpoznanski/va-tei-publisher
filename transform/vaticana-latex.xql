...
module namespace model="http://www.tei-c.org/pm/models/vaticana/latex";
    ...
    $input !         (
            ...
            return
                    ...
                    case element(persName) return
                        latex:inline($config, ., ("tei-persName", "annotation", "annotation-role", "authority", css:map-rend-to-class(.)), .)
                    case element(settlement) return
                        latex:alternate($config, ., ("tei-settlement", css:map-rend-to-class(.)), ., ., .)
                    case element(affiliation) return
                        latex:link($config, ., ("tei-affiliation", css:map-rend-to-class(.)), ., concat('https://va.wiki.kul.pl/wiki/Item:', substring-after(@ref, '-')), map {})
                    case element(measure) return
                        latex:alternate($config, ., ("tei-measure", css:map-rend-to-class(.)), ., ., concat('Jednostka: ', @unit, ', Ilość: ', @quantity, ', Szczegóły: ', @commodity))
                    case element(orgName) return
                        latex:alternate($config, ., ("tei-orgName", css:map-rend-to-class(.)), ., ., .)
                    case element(placeName) return
                        latex:alternate($config, ., ("tei-placeName", css:map-rend-to-class(.)), ., ., .)
                    ...
                    case element(section) return
                        if (@type='text') then
                            latex:inline($config, ., ("tei-section1", css:map-rend-to-class(.)), .)
                        else
                            if (@type='formularz') then
                                latex:inline($config, ., ("tei-section2", css:map-rend-to-class(.)), .)
                            else
                                $config?apply($config, ./node())
                    case element(education) return
                        latex:alternate($config, ., ("tei-education", css:map-rend-to-class(.)), ., ., @subtype)
                    case element() return
