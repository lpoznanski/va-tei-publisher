...
module namespace model="http://www.tei-c.org/pm/models/vaticana-demo/web";
            ...
            return
                    ...
                    case element(teiHeader) return
                        if ($parameters?header='short') then
                            ...
                            (
                                html:block($config, ., ("tei-teiHeader4", css:map-rend-to-class(.)), //publicationStmt/p)                                => model:map($node, $trackIds),
                                html:listItem($config, ., ("tei-teiHeader5", css:map-rend-to-class(.)), //sourceDesc, 'Źródła:')                                => model:map($node, $trackIds),
                                html:listItem($config, ., ("tei-teiHeader6", css:map-rend-to-class(.)), //date, 'Data wydania:')                                => model:map($node, $trackIds),
                                html:listItem($config, ., ("tei-teiHeader7", css:map-rend-to-class(.)), //placeName, 'Miejsce wydania:')                                => model:map($node, $trackIds)
                            )
                    ...
                    case element(body) return
                        (
                            html:block($config, ., ("tei-body1", css:map-rend-to-class(.)), root(.)//fileDesc/titleStmt/substring-before(title, '-'))                            => model:map($node, $trackIds),
                            html:link($config, ., ("tei-body2", css:map-rend-to-class(.)), root(.)//fileDesc/titleStmt/substring-after(title, '- '), concat('https://va.wiki.kul.pl/wiki/Item:', root(.)//fileDesc/titleStmt/substring-after(title, '- ')), (), map {})                            => model:map($node, $trackIds),
                            html:block($config, ., ("tei-body3", css:map-rend-to-class(.)), root(.)//profileDesc/abstract)                            => model:map($node, $trackIds),
                            html:section($config, ., ("tei-body4", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                        )
                    ...
                    case element(persName) return
                        html:alternate($config, ., ("tei-persName", css:map-rend-to-class(.)), ., ., ., map {})                        => model:map($node, $trackIds)
                    case element(affiliation) return
                        html:alternate($config, ., ("tei-affiliation", css:map-rend-to-class(.)), ., ., ., map {})                        => model:map($node, $trackIds)
                    case element(placeName) return
                        if (parent::p) then
                            html:alternate($config, ., ("tei-placeName", css:map-rend-to-class(.)), ., ., ., map {})                            => model:map($node, $trackIds)
                        else
                            $config?apply($config, ./node())
