...
module namespace model="http://www.tei-c.org/pm/models/vaticana-demo/latex";
    ...
    $input !         (
            ...
            return
                    ...
                    case element(body) return
                        (
                            latex:block($config, ., ("tei-body1", css:map-rend-to-class(.)), root(.)//fileDesc/titleStmt/substring-before(title, '-')),
                            latex:link($config, ., ("tei-body2", css:map-rend-to-class(.)), root(.)//fileDesc/titleStmt/substring-after(title, '- '), concat('https://va.wiki.kul.pl/wiki/Item:', root(.)//fileDesc/titleStmt/substring-after(title, '- ')), map {}),
                            latex:block($config, ., ("tei-body3", css:map-rend-to-class(.)), root(.)//profileDesc/abstract),
                            latex:section($config, ., ("tei-body4", css:map-rend-to-class(.)), .)
                        )
                    ...
                    case element(persName) return
                        latex:alternate($config, ., ("tei-persName", css:map-rend-to-class(.)), ., ., .)
                    case element(affiliation) return
                        latex:alternate($config, ., ("tei-affiliation", css:map-rend-to-class(.)), ., ., .)
